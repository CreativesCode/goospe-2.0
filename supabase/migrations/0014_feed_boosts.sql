-- 0014 — Integra boosts activos al feed.
-- Un boost activo (status='active' y now() dentro de [starts_at, ends_at]) sube el lugar
-- al tope del feed (bonus al score) y marca `boosted=true` para que la UI muestre "Destacado".
-- Cambia la firma de retorno (nueva columna) → drop + create.

drop function if exists public.get_feed(double precision, double precision, integer, integer, integer);
...
as $$
  ...
  select
    ...
    ( 0.60 * exp(-c.dist_m / 2500.0)
    + 0.30 * coalesce(st.trending_score, 0) / 5.0
