-- 0012 — RPC del feed (Fase 2). Candidatos por cercanía (PostGIS) + score sin LLM por scroll.
-- Score = distancia (decay exponencial) + trending + término de diversidad estable por lugar.
-- La afinidad por embedding (taste_profiles) se sumará cuando haya usuarios; hoy el feed
-- funciona anónimo (cercanía + popularidad).
...
returns table (
  ...
  category_emoji text, category_name text, score double precision
)
language sql stable
